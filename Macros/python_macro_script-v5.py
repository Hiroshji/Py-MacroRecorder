...
def start_recording():
    global keyboard_listener, mouse_listener, recorded_events, is_recording
    if is_recording:
        return
    ...
    status_label.config(text="Recording... Press F6 to stop.")
    start_button.config(text="Stop Recording (F6)")

# Function to stop recording
def stop_recording():
    global keyboard_listener, mouse_listener, is_recording
    if not is_recording:
        return
    ...
    start_button.config(text="Start Recording (F6)")
...
def replay_events():
    ...
    if not loaded_events:
        status_label.config(text="No events to replay.")
        return
...
# Function to handle F6 and F7 key presses
...
start_button = tk.Button(root, text="Start Recording (F6)", command=start_recording)
start_button.pack(pady=10)

replay_button = tk.Button(root, text="Replay Events (F7)", command=replay_events)
replay_button.pack(pady=10)

status_label = tk.Label(root, text="Press 'Start Recording (F6)' to begin.")
status_label.pack(pady=10)

# Set up keyboard listener for F6 and F7 keys
keyboard_listener_global = keyboard.Listener(on_press=on_key_press)
